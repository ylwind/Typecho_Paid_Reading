<?php
/**
 * Tee Pay For Typecho付费阅读插件_统一支付平台版本</a>
 * @package TeePay For Typecho
 * @author 杨柳清风博客
 * @version 1.0.0
 * @link https://blog.ylwind.cn
 * @date 2021-01-24
 */
class TeePay_Plugin implements Typecho_Plugin_Interface{
		...
		if($row['teepay_isFee']=='y'){
			if($row['authorId']!=Typecho_Cookie::get('__typecho_uid')){
				...
				<div style="background:#f8f8f8;padding:30px 20px;border:1px dashed #ccc;position: relative;z-index:999;margin:15px 0">
					...
					<span style="position: absolute;top:5px;left:15px;font-size:90%;color:#90949c;">付费内容已解锁 | 订单号：<?php echo $rowItem['feeid'] ?></span>
				...
				<?php }else{ ?>
					<div style="background:#f8f8f8;padding:35px 15px 10px;border:1px dashed #ccc;position: relative;text-align:center;margin:15px 0;">
						<form id="teepayPayPost" onsubmit="return false" action="##" method="post" style="margin:10px 0;">
							<?php if($option->show_Alipay_Wxpay == "union"){ ?>
                                <input type="radio" id="feetype1" name="feetype" value="alipay" style="display:none;">
                                <input type="radio" id="feetype2" name="feetype" value="alipay" style="display:none;">
                                <input type="radio" id="feetype3" name="feetype" value="union" checked>&nbsp;&nbsp;支付宝/微信支付
							<?php }else{
						...
						<div style="clear:left;"></div>
						<span>温馨提示：免登录付款<?php echo $cookietime;?>天内可阅读，登录</span>用户付款后可永久阅读 </span><br/>
						<span>本服务由<span style="color: red"><a href="https://cashier.5nm.top/user">杨柳清风PAY</a></span>提供技术支持</span>
    ...
    public static function config(Typecho_Widget_Helper_Form $form){
		...
		$alipay_notify_url = new Typecho_Widget_Helper_Form_Element_Text('alipay_notify_url', array('value'), $plug_url.'/TeePay/alipay_notify_url.php', _t('支付宝异步回调接口(一般不用修改)'), _t('支付完成后异步回调的接口地址。'));
        $form->addInput($alipay_notify_url);
		//payjs配置	
		$payjs_wxpay_mchid = new Typecho_Widget_Helper_Form_Element_Text('payjs_wxpay_mchid', array('value'), "", _t('统一支付平台商户号'), _t('在<a href="https://cashier.5nm.top/user" target="_blank">统一支付平台官网</a>注册的商户号。'));
        $form->addInput($payjs_wxpay_mchid);
		$payjs_wxpay_key = new Typecho_Widget_Helper_Form_Element_Text('payjs_wxpay_key', array('value'), "", _t('统一支付平台密钥'), _t('在<a href="https://cashier.5nm.top/user" target="_blank">统一支付平台官网</a>注册的通信密钥。'));
        $form->addInput($payjs_wxpay_key);
		$payjs_wxpay_notify_url = new Typecho_Widget_Helper_Form_Element_Text('payjs_wxpay_notify_url', array('value'), $plug_url.'/TeePay/wxpay_notify_url.php', _t('异步回调接口(一般不用修改)'), _t('支付完成后异步回调的接口地址。'));
        $form->addInput($payjs_wxpay_notify_url);	
		//设置显示微信，支付宝，还是全部都显示
		$show_Alipay_Wxpay= new Typecho_Widget_Helper_Form_Element_Radio('show_Alipay_Wxpay',array('union' => _t('聚合支付'),'alipay' => _t('仅支付宝'),'wxpay' => _t('仅微信支付')),'all',_t('支付方式'),_t("直接选择聚合支付方式即可"));
	...
	public static function createTableTeePayFee($db){
		...
		$db->query('CREATE TABLE IF NOT EXISTS `'.$prefix.'teepay_fees` (
		  ...
		  `feetype` enum("alipay","wxpay","union","tlepay") COLLATE utf8_general_ci DEFAULT "alipay",
